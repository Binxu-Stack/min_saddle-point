...
MinSaddleHFTN::MinSaddleHFTN(LAMMPS *lmp) : MinSaddle(lmp)
{
  searchflag = 1;
  ...
  _fpPrint = NULL;
  vector_exist = NUM_HFTN_ATOM_BASED_VECTORS;
...
MinSaddleHFTN::~MinSaddleHFTN (void)
{
  for (int  i = 1; i < NUM_HFTN_ATOM_BASED_VECTORS; i++)
...
void MinSaddleHFTN::setup_style()
{
...
void MinSaddleHFTN::reset_vectors()
{
  nvec = 3 * atom->nlocal;
...
------------------------------------------------------------------------- */
int MinSaddleHFTN::execute_hftn_(const bool      bPrintProgress,
...
bool MinSaddleHFTN::compute_inner_cg_step_(const double    dTrustRadius,
...
double MinSaddleHFTN::calc_xinf_using_mpi_(void) const
{
...
double MinSaddleHFTN::calc_dot_prod_using_mpi_(const int  nIx1,
...
double MinSaddleHFTN::calc_grad_dot_v_using_mpi_(const int  nIx) const
{
...
void MinSaddleHFTN::calc_dhd_dd_using_mpi_(double &  dDHD,
                                     double &  dDD) const
{
...
void MinSaddleHFTN::calc_ppnew_pdold_using_mpi_(double &  dPnewDotPnew,
...
void MinSaddleHFTN::calc_plengths_using_mpi_(double &  dStepLength2,
...
bool MinSaddleHFTN::step_exceeds_TR_(const double    dTrustRadius,
                               const double    dPP,
                               const double    dPD,
                               const double    dDD,
                               double &  dTau) const
...
bool MinSaddleHFTN::step_exceeds_DMAX_(void) const
{
...
void MinSaddleHFTN::adjust_step_to_tau_(const double tau)
{
  if (nextra_global) {
    double *  pGlobal  = _daExtraGlobal[VEC_CG_P];
    double *  dGlobal  = _daExtraGlobal[VEC_CG_D];
...
double MinSaddleHFTN::compute_to_tr_(const double  dPP,
                               const double  dPD,
                               const double  dDD,
...
void MinSaddleHFTN::evaluate_dir_der_(const bool      bUseForwardDiffs,
                                const int       nIxDir,
                                const int       nIxResult,
...
void MinSaddleHFTN::open_hftn_print_file_(void)
{
  int  nMyRank;
  MPI_Comm_rank (world, &nMyRank);

  char  szTmp[50];
  sprintf (szTmp, "progress_MinSaddleHFTN_%d.txt", nMyRank);
  _fpPrint = fopen (szTmp, "w");
  if (_fpPrint == NULL) {
    printf ("*** MinSaddleHFTN cannot open file '%s'\n", szTmp);
    printf ("*** continuing...\n");
    return;
  }
...
void MinSaddleHFTN::hftn_print_line_(const bool    bIsStepAccepted,
...
void MinSaddleHFTN::close_hftn_print_file_(void)
{
