...
#include <cmath>
#include "min_saddle_linesearch.h"
#include "atom.h"
#include "update.h"
#include "neighbor.h"
#include "domain.h"
...
MinSaddleLineSearch::MinSaddleLineSearch(LAMMPS *lmp) : MinSaddle(lmp)
{
  searchflag = 1;
  gextra = hextra = NULL;
...
MinSaddleLineSearch::~MinSaddleLineSearch()
{
  delete [] gextra;
  delete [] hextra;
  delete [] x0extra_atom;
...
void MinSaddleLineSearch::init()
{
  MinSaddle::init();

  if (linestyle == 0) linemin = &MinSaddleLineSearch::linemin_backtrack;
  else if (linestyle == 1) linemin = &MinSaddleLineSearch::linemin_quadratic;
  else if (linestyle == 2) linemin = &MinSaddleLineSearch::linemin_forcezero;
...
void MinSaddleLineSearch::setup_style()
{
...
void MinSaddleLineSearch::reset_vectors()
{
  // atomic dof
...
int MinSaddleLineSearch::linemin_backtrack(double eoriginal, double &alpha)
{
  int i,m,n;
...
int MinSaddleLineSearch::linemin_quadratic(double eoriginal, double &alpha)
{
  int i,m,n;
  double fdothall,fdothme,hme,hmax,hmaxall;
  double de_ideal,de;
...
int MinSaddleLineSearch::linemin_forcezero(double eoriginal, double &alpha)
{
  int i,m,n;
  double fdothall,fdothme,hme,hmax,hmaxall;
  double de;
...
double MinSaddleLineSearch::alpha_step(double alpha, int resetflag)
{
  int i,n,m;
...
double MinSaddleLineSearch::compute_dir_deriv(double &ff)
{
   int i,m,n;
   double *hatom, *fatom;
   double dot[2],dotall[2];
